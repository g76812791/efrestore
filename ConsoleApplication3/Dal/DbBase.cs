...
namespace DbHelp.Base
{
    public class DbBase
    {
        protected  DbContext MasterDb = new myDataBaseEntities();
        #region 自定义其他方法
        /// <summary>
         ...
         public List<TModel> Query<TModel>(string sql, List<SqlParameter> parms, CommandType cmdType = CommandType.Text)
        {
            ...
            if (cmdType == CommandType.StoredProcedure)
            {
                ...
                {
                    paraNames.AppendFormat(" @{0},",sqlPara);
                }
                if (paraNames.Length > 0)
                {
                    sql = string.Format("exec {0} {1}", sql, paraNames.ToString().Trim(','));
                }
                else
                {
                    sql = "exec " + sql;
            ...
            var list = MasterDb.Database.SqlQuery<TModel>(sql, parms.ToArray());
    ...
    public class DbBase<T> : DbBase where T : class, new()
    {
        ...
        public T FirstOrDefault(Expression<Func<T, bool>> whereLambda = null)
        {
            if (whereLambda == null)
            {
                return MasterDb.Set<T>().FirstOrDefault();
            }
            return MasterDb.Set<T>().FirstOrDefault(whereLambda);
        }
        ...
        public List<T> GetAll(string ordering = null)
        {
            return ordering == null
                ? MasterDb.Set<T>().ToList()
                : MasterDb.Set<T>().OrderBy(ordering).ToList();
        }
        ...
        {
            var iQueryable = MasterDb.Set<T>().Where(whereLambda);
            return ordering == null
                ? iQueryable.ToList()
        ...
        {
            return whereLambda == null ? MasterDb.Set<T>() : MasterDb.Set<T>().Where(whereLambda);
        }
        ...
        {
            return whereLambd == null ? MasterDb.Set<T>().Count() : MasterDb.Set<T>().Where(whereLambd).Count();
        }
        ...
        {
            return MasterDb.Set<T>().Where(whereLambd).Any();
        }
        ...
        public List<T> Page<TKey>(int pageIndex, int pageSize, out int rows, Expression<Func<T, TKey>> orderBy, Expression<Func<T, bool>> whereLambda = null, bool isOrder = true)
        {
            IQueryable<T> data = isOrder ?
                MasterDb.Set<T>().OrderBy(orderBy) :
                MasterDb.Set<T>().OrderByDescending(orderBy);
        ...
        public List<T> Page(int pageIndex, int pageSize, out int rows, string ordering, Expression<Func<T, bool>> whereLambda = null)
        {
            ...
            var data = MasterDb.Set<T>().OrderBy(ordering);
            if (whereLambda != null)
            {
                data = data.Where(whereLambda);
            }
        ...
        {
            return MasterDb.Set<T>().Where(whereLambda).Select<TDto>().ToList();
        }
